...
7. **Balances (staff):** quota per type per year (IST calendar or academic year, configured); decremented on approval by working days consumed (the calendar resolver, TTM-FR-28, evaluated **for the applicant's School** — university holidays and the School's non-working weekdays inside the range don't count, so a Nursing staff member's Sunday is a working day while an Engineering colleague's is not); **half-days decrement 0.5** — balances are tracked in half-day precision, a half-day on a holiday consumes nothing, and the HR export carries the 0.5 granularity; restored on cancellation for unused days; an application exceeding the balance is **flagged `exceeds-balance`, not blocked** — the approver decides with the flag visible, and the excess appears in the HR export (loss-of-pay is HR's business).
...
- Localization: IST, DD-MM-YYYY, working days from the calendar resolver (TTM-FR-28), resolved per School.
...
- LVE-FR-06: Staff balances: per-type annual quota; working-day decrement on approval (calendar resolver TTM-FR-28, per the applicant's School) with half-day precision (0.5 per half-day, business rule 7); restore on cancellation; `exceeds-balance` flagging (never blocking); balance visible to the owner and their approver; HR export in half-day granularity.
...
- The TTM calendar resolver (TTM-FR-28) is the single working-day source for balance arithmetic and retro windows — university holidays plus the applicant's School working-day pattern. This module never computes working days itself.
