...
- **Period instances are dated by the calendar resolver (TTM-FR-28), not by the weekday alone.** A published timetable entry is a weekly recurrence; the dated Sessions it produces exist only on dates the resolver calls a teaching day for that School — so university holidays and non-working weekdays generate no Session and no `not-captured` flag, a weekend-teaching School (Nursing, Medicine) generates Saturday and Sunday Sessions, and a compensatory working day runs the Sessions of the weekday it follows. ATT never re-derives working days itself; a second implementation would drift from the one leave balances and task recurrence use.
- **A date carrying captured attendance cannot later be declared a holiday** (TTM §4 rule 13). ATT supplies the checker TTM consults, so the guard is enforced with ATT's own data rather than a copy of it.
