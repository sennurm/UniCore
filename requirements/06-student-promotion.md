...
Module code: PRM · Status: DRAFT — pending approval · Last updated: 2026-07-30
...
- PRM-FR-03: **Promotion run per Program + Batch + position** (locked 30-07-2026; previously per Program alone): compute attendance % (ATT, captured Sessions only), credits, and backlog count per student; snapshot inputs; assign each student to eligible / eligible-with-exceptions / not-eligible. A Programme always has three or four batches in flight at once, so "per Program" never identified a runnable set — the run targets the batch that is finishing a given position. The **position**, not the batch, is what the run advances: a detained student keeps their original batch but is picked up by whichever run covers their current position (PRM-FR-19), so the run's roster is *students of this Programme at this position*, which the batch names but does not define.
- PRM-FR-18: **Eligible students are pre-marked promoted.** When a run completes, every student computed eligible is already marked for promotion; staff touch only the exceptions. This changes the default, not the authority: the configured approval chain (PRM-FR-06) and ratification (PRM-FR-10) still execute in full, so the UGC attendance gate, the School Incharge's ratification, term closure and the audit trail are all unchanged. Marking a pre-marked student as not-promoted requires the same recorded reason as any other exception.
- PRM-FR-19: **Detention diverges position from batch.** A detained student retains their original batch for records, reporting and graduation — their admission year is a fact, not a status — while their position falls a term behind. Because runs key on position, they rejoin the next run covering that position, travelling with the batch below without their history being rewritten. Batch and position are therefore both stored; neither is derivable from the other once a detention has occurred.
...
- PRM-FR-17: **Attendance freeze:** run trigger publishes the freeze event per business rule 10, scoped to **the Sections of the batch being promoted** — not the whole Programme (corrected 30-07-2026). Freezing every Section of a Programme meant that promoting the outgoing final-year batch also froze attendance corrections for its first-years, mid-term, for a run that had nothing to do with them. The freeze covers exactly the Sections whose students are in the run; consumed by ATT (correction window), LVE/ATT (retro-marking exemption boundary), and AUTH (edge-case reasoning). Freeze state is queryable per Program/term; lifting (run discard before any ratification) is School Incharge-approved and audited.
...
| Detained student's batch and position diverge | **DECISION:** the batch is identity and never changes — their admission year is a fact. The **position** falls a term behind, and runs key on position, so they are picked up by the next run covering it and travel with the junior cohort. Batch and position are both stored; after any detention neither derives from the other (PRM-FR-19). |
| A run is triggered for a batch whose students sit at two different positions | **DECISION:** the run targets (Programme, position) and reports the students of that batch at other positions as out-of-run, naming them. Silently including them would advance students who have not completed the term being assessed. |
| Attendance freeze while another batch of the same Programme is mid-term | **DECISION:** the freeze covers only the Sections holding the run's students (PRM-FR-17). Promoting the final-year batch must not freeze first-year attendance corrections. |
| Every student in the run is eligible | **DECISION:** all are pre-marked promoted (PRM-FR-18) and the chain still runs; ratification is never skipped just because nothing needs attention. The approver sees a register with zero exceptions, not an empty one. |
| An approver un-marks a pre-marked student | **DECISION:** treated as an override — mandatory reason, audited, same as any other exception. The pre-mark is a computed default, not an approval. |
...
| Worst case: wrong criteria configured, discovered after ratification of a whole Program | **DECISION:** mass rollback uses the same School Incharge+Registrar flow applied per run, still bounded by the attendance-capture window; students already past the window are handled case-by-case via next-term overrides. Configuration changes require a second School Incharge confirmation on save to reduce recurrence. |
...
- Ratification commit: atomic per student; a mid-run failure leaves every student either fully committed or fully uncommitted — no partial student state; the run is resumable.
...
| TC-PRM-020 | Mid-run commit failure leaves no partial student | NFR | P1 | Ratification run; induced failure at student k | Ratify; inspect states | Students < k committed, ≥ k uncommitted; run resumable; no half-advanced student | §9, PRM-FR-10 |
| TC-PRM-024 | Run targets one batch, not the Programme | Happy | P0 | BT-CSE with batches 2023, 2024, 2025, 2026 in flight | Trigger a run for batch 2024 at its current position | Only that batch's students at that position are in the register; the other three batches are untouched | PRM-FR-03 |
| TC-PRM-025 | Freeze covers only the run's Sections | Boundary | P0 | Run triggered for the final-year batch | Attempt a routine attendance correction on a first-year Section | Correction succeeds — that Section is not frozen; a correction inside the run's Sections is blocked | PRM-FR-17, §8 |
| TC-PRM-026 | Eligible students arrive pre-marked | Happy | P0 | Run with 200 eligible, 12 exceptions | Open the register | All 200 already marked promoted; staff action needed only on the 12 | PRM-FR-18 |
| TC-PRM-027 | Pre-marking does not skip the chain | Access | P0 | Run with every student eligible | Attempt to ratify without completing the chain | Refused; the configured chain must complete first, then ratification commits | PRM-FR-18, PRM-FR-06 |
| TC-PRM-028 | Un-marking requires a reason | Negative | P0 | Pre-marked eligible student | Approver un-marks without a reason | Rejected; with a reason it is recorded as an audited override | PRM-FR-18 |
| TC-PRM-029 | Detained student keeps their batch | Boundary | P0 | Student of BT-CSE-2024 detained at semester 3 | Inspect the record after ratification | Batch still BT-CSE-2024; position still 3; batch unchanged by the detention | PRM-FR-19 |
| TC-PRM-030 | Detained student joins the next run for their position | Happy | P0 | The above student; next term's run covers semester 3 | Trigger that run | The detained student appears in it alongside the junior batch | PRM-FR-19 |
