...
Module code: AUTH · Status: DRAFT — pending approval · Last updated: 2026-07-30
...
| Students | Hold the `student` role scoped to their Programme; registered-device management (request change). Their Section comes from ONB membership, not from a grant |
...
- **Singleton leadership roles per org unit:** designated roles allow at most **one active grant per org unit** — one HoD per Department, one School Incharge per School, one Faculty Dean per Faculty Division, one Class In-charge per Section, and one VC / Registrar / Dean Academic Affairs / Chancellor per University (the Chancellor is a top-of-hierarchy role introduced by the Leave module, see 10-leave-management.md). **`pro-chancellor` is deliberately exempt from the singleton rule** — the university's leadership page documents two simultaneous holders (28-07-2026), so either may approve. Granting a second active holder is rejected; replacing the holder (e.g., a permanent HoD arriving to end an additional charge) uses the **supersede flow**: old grant revoked and new grant issued in one atomic, audited operation, so the unit is never leaderless and never double-headed.
...
- **The `student` role is Programme-scoped (locked 31-07-2026).** Students are inside the RBAC model like everyone else — a student holding no grant can authenticate and do nothing, which is not a security posture but an omission. The grant binds to the **Programme**, deliberately *not* to the Section: a Section is a per-term instance, so a Section-scoped student grant would have to be revoked and reissued for every student at every term closure (~15,000 writes twice a year) and would duplicate the dated Section membership ONB already owns as the single source of truth (ONB-FR-10). Programme is stable for the whole degree; "which class is this student in today" is a membership question, not a grant question. The role is **not** term-bound and survives promotion — a student advancing from semester 3 to 4 stays in the same Programme.
- **Student grants are issued by import, not by hand.** An intake of 15,000 cannot be granted a role one screen at a time, so ONB grants it in the same transaction that provisions the student (ONB-FR-23). Issuing is idempotent: re-importing a file never produces a second grant, and does backfill students provisioned before the role existed.
...
|---|---|---|---|
| `student` | **Programme** | Granted automatically by the ONB import that provisions the student (`office-staff` / System Admin / Super Admin may issue it) | ONB, ATT, TTM and PRM student-facing views |
...
| `office-staff` (Admin / office staff) | School | School Incharge | ONB imports, single adds, allotment/re-allotment, withdrawal within the School subtree; TTM term-calendar upload |
...
| `pro-chancellor` | University (**not** singleton — two holders documented 28-07-2026) | University governance | LVE approvals between VC/Registrar and the Chancellor |
...
| Create/rename/re-parent/deactivate/reactivate org units (Faculty Division/School/Department/Program) | Super Admin only; deactivate-never-delete; all changes audited | API + service layer |
| Bulk-import org structure (course catalogue CSV) | Super Admin only; partial commit with an error report | API + service layer |
| Read the user & role directory (incl. CSV export) | Super Admin, System Admin (own scope) | API (scope filter) |
| Bulk role update via CSV upload | Super Admin, System Admin; each change re-validated against singleton + scope rules and audited individually | API + service layer |
...
- AUTH-FR-04: RBAC with scoped, time-bound grants; permission = union of grants; deny by default. **Every user group is inside the model, students included** — there is no class of user that holds no role.
- AUTH-FR-21: **The role registry is served, never hardcoded.** Clients read the catalog (code, display name, scope unit type, singleton and term-bound flags) from the API rather than carrying a literal list, so a role added to the registry is immediately grantable and a client list cannot drift out of date. The unit type is part of the contract because it tells the grant screen what kind of org unit the next field must name.
...
- AUTH-FR-18: **Reporting-chain configuration** (University-level, role-based): Class In-charge → HoD → School Incharge → Faculty Dean → Dean Academic Affairs → VC → **Pro-Chancellor** → Chancellor, with Registrar → Pro-Chancellor; non-academic staff resolve via the registry's unit-head map. **Principal is not a chain tier** — it is a School-level title ("equal to school incharge" per the university structure document, 28-07-2026), so Principals route as School Incharges to their Faculty Dean. Exposed as a resolution API ("who is X's reporting person") consumed by LVE routing/cascade (10-leave-management.md) and TSK escalation/visibility. The API also reports each level's **holder status (active / on-approved-leave / vacant)** so consumers can cascade past on-leave or vacant levels. Changes are audited; the chain must be acyclic with Chancellor as the unique terminal.
- AUTH-FR-20: **User & role directory:** a single filterable view listing every account with its active grants (filter by search, role, and account status), from which grants are issued and revoked inline. Exportable as CSV and **re-importable**: the export carries a `roles` column of `role@org-unit-path` entries (`;`-separated); on upload, entries added are granted and entries removed are revoked. Bulk changes are **not** a bypass — every grant/revoke passes the same singleton, scope, validity and audit rules as a single-grant operation, and per-row failures are reported without failing the run.
- AUTH-FR-19: **Org-structure administration:** Super Admin creates, renames, re-parents, deactivates and reactivates org units (Faculty Division, School, Department, Program), one at a time from the org table or in bulk via the flat course-catalogue CSV (one row per Programme; ancestors auto-created; Programme level/duration/mode stored). **Exactly one University** exists — a second is refused. **Department is optional in the catalogue import**: Schools without departments get a default Department mirroring the School, flagged `auto_created` so the org table distinguishes structural placeholders from real academic units (28-07-2026). Programmes additionally carry category, industry partner, internship months and lateral-entry semester. **Every School declares its curriculum cadence (`semester` | `yearly`) at creation — mandatory, no default inferred** (locked 30-07-2026); Programmes inherit it and may carry their own override for genuinely mixed catalogues (a School running B.Pharm alongside PhD programmes). The **effective cadence** of a Programme is its own value if set, else its School's, and it determines the Programme's position ladder (`duration_years × 2` for semester, `duration_years` for yearly) — which ONB validates positions against, TTM generates Sections from, and PRM advances. Cadence is editable afterwards by Super Admin but **the change is refused while any student sits at a position that the new cadence's ladder does not contain**, since silently rewriting positions would move real students. Schools created before this rule (the 13 seeded from the catalogue) are migrated to `semester` and flagged `cadence_unconfirmed` until a School Incharge confirms. Unit codes are immutable once created (they are embedded in descendant ltree paths); a mistaken code is deactivated and replaced. Units are never hard-deleted (history and scoped grants hang off them); deactivation blocks new grants/imports against the unit while preserving reads. All changes audited with before/after. Section instances are NOT managed here — they are per-term entities created by the Timetable Cell during term setup (TTM-FR-19).

## 8. Edge Cases, Worst Cases & Decisions

| Case | Decision |
|---|---|
| A provisioned user holds no role at all | **DECISION:** they can authenticate and nothing else — every permission check denies. This is correct as a *failure* mode but must never be the *normal* state of a whole user group, which is what happened to students before the `student` role existed. Provisioning grants the role in the same transaction. |
| Student is re-allotted to another Section mid-term | **DECISION:** no grant change. The `student` grant is Programme-scoped and the Section comes from dated membership (ONB-FR-10), so a re-allotment moves the membership and leaves authorization untouched. |
| Student is promoted to the next semester/year | **DECISION:** no grant change — same Programme, so the grant survives. Only the position advances. Contrast `class-incharge`, which *is* term-bound and is revoked by term closure. |
| Student transfers to a different Programme | **DECISION:** the grant follows the Programme — the old one is revoked and a new one issued for the new Programme, in the same audited operation as the transfer (ONB-FR-11). A student must never hold a grant on a Programme they have left. |
| Student withdraws | **DECISION:** sessions are revoked immediately and the account deactivated (ONB-FR-12). The grant is left in place rather than deleted, so history stays readable; deactivation is what denies access, and re-joining reactivates the same account and grant. |
| Same person is staff and a student | **DECISION:** both grants coexist, per the multi-role rule above. Permissions never merge across the student/staff boundary in one request — the app presents a role context switcher. |
...
- ~~Campus Principals/Directors~~ — **resolved 28-07-2026**: Principal is a School-level title ("equal to school incharge"), not a campus tier; removed from the reporting chain.
- ~~"Dean FSC (confirm)" and the Agri Division label~~ — **resolved 28-07-2026** by the university structure document: **Faculty of Sciences** and **Faculty of Agricultural Sciences**.
- **Faculty count**: the structure document's body lists seven Faculties (incl. Medicine) but its summary says six — confirm seven.
- **School of Medicine** (Takshashila Medical College) appears as a School with no programmes or head; confirm whether UniCore manages it at all or it stays an external institution.
- **No Chancellor is currently named** in the university's leadership page — the chain terminates there, so leave from the VC/Registrar would rest with a vacant role (reported as `vacant`, alerting per LVE-FR-03). Confirm the incumbent.
- **Programme specialisations/streams** (M.Tech CSE → AI / Big Data / IoT; MBA's 16 areas; M.MLS streams) are not modelled — they live in the programme name. Decide whether they become elective groups, Sections, or their own level before TTM electives are built.
...
| TC-AUTH-023 | Second University refused | Negative | P0 | A university root exists | Create another university unit | 409 citing the single-University rule; attempt audited | AUTH-FR-19 |
| TC-AUTH-024 | Flat catalogue import creates the whole branch | Happy | P0 | University exists; catalogue CSV with one Programme row | Import | Faculty Division, School, Department and Programme created; Programme carries level/duration/mode; re-import reports all unchanged | AUTH-FR-19 |
| TC-AUTH-025 | Org unit deactivated, not deleted | Access | P0 | Programme in use | Deactivate it, list units, attempt DELETE | Hidden from the default list, visible with include-inactive, restorable; no delete route exists (405); history intact | AUTH-FR-19 |
| TC-AUTH-026 | Role CSV round-trip grants and revokes | Happy | P0 | User A with no roles, user B holding `hod@dept` | Export directory, add a role for A, clear B's, re-upload | A's grant issued, B's revoked, both audited; unchanged rows untouched | AUTH-FR-20 |
| TC-AUTH-027 | Role CSV errors are per-row | Negative | P0 | CSV naming an unknown user, an unknown role and a bad org path | Upload | Valid rows applied; three rows rejected with reasons; run not aborted | AUTH-FR-20 |
| TC-AUTH-028 | Imported students hold the Student role | Happy | P0 | Student CSV for a Programme | Run the import | Every created student holds `student` scoped to that Programme, granted in the same transaction and audited | AUTH-FR-04, ONB-FR-23 |
| TC-AUTH-029 | Student grant is Programme-scoped, not Section | Boundary | P0 | Imported student in Section "I Semester - A" | Read their grants | Exactly one grant, on the Programme; no Section-scoped grant exists | §4 RBAC model |
| TC-AUTH-030 | Re-import does not duplicate the grant | Boundary | P0 | Student already holds the role | Re-upload the same file | Still exactly one active `student` grant; a student provisioned before the role existed gains one | ONB-FR-23 |
| TC-AUTH-031 | Term closure leaves student grants alone | Boundary | P0 | Section with a Class In-charge and imported students; PRM publishes term closure | Inspect grants | `class-incharge` revoked; every `student` grant still active | §4 term-bound rule |
| TC-AUTH-032 | Role registry is served to clients | Happy | P1 | Any admin session | GET the role catalog | Every registry role returned with its scope unit type, singleton and term-bound flags — including roles added after the client shipped | AUTH-FR-21 |
| TC-AUTH-033 | Role catalog read is permission-gated | Access | P0 | Session without `rbac:read` | GET the role catalog | 403; the catalog is not public | AUTH-FR-05, AUTH-FR-21 |

Coverage: all §6 acceptance criteria, the §4 authorization matrix (incl. org-unit CRUD TC-010, single-University TC-023, catalogue import TC-024, deactivate-never-delete TC-025, role CSV round-trip TC-026/027), DPDP erasure/consent obligations (§5), term-bound grant lifecycle (TC-015/016/018), singleton + additional-charge rules (TC-019–022), and every §8 edge case except SMS-gateway failover (covered operationally, add TC in integration phase) map to at least one test. Minor-consent tests were removed with the 18+ policy lock (00-overview.md §7).
