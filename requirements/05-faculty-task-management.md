...
| Curate School tag catalog (rename / merge / deactivate) | School Dean or School Admin (own School); deactivate-only, never delete | API + service layer |
...
- TSK-FR-21: **School tag curation** by Dean/School Admin: rename (entity-wide, audited with old name), merge (re-point tasks to survivor; over-limit results allowed and flagged in the merge summary), deactivate (hidden from pickers, retained on tasks, filterable); no delete; all curation actions audited.
...
| Tag name containing personal/sensitive content (e.g., 'disciplinary-drrao') | **DECISION:** policy rule (no personal data in tag names) surfaced as a UI hint; not machine-enforced in MVP; Dean rename/merge is the remedy, audited. |
| Tag sprawl despite autocomplete | **DECISION:** Dean/School Admin merge + deactivate are the cleanup tools (entity semantics — reports heal retroactively); no automatic similarity policing in MVP. |
