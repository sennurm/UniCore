...
UniCore gives university leadership a hierarchical task system for non-teaching work: **Principal / Faculty Dean / School Incharge → HoD → Faculty Member**. An assigner can only assign downward within their org scope; skip-level assignment (a Faculty Dean directly to a Faculty Member) is legal, with the assignee's HoD given read visibility. VC/Registrar are deliberately **not** assigners — leadership routes work through Principals, Faculty Deans, and School Incharges — but the escalation chain tops out at the **VC/Pro-VC office**, so Principal-assigned overdue tasks always have somewhere to go. Task kinds are a **School-configurable taxonomy** (seeded with administrative duty, exam duty, event coordination, report submission). Tasks carry due dates, P1/P2/P3 priorities, a status lifecycle (assigned → accepted/in-progress → submitted-for-review → completed | reopened), attachments, a comments thread, configurable reminders, and overdue escalation. **Full recurrence ships in MVP**: fixed-interval cycles that are holiday-aware (campus calendar) and support per-cycle assignee rotation. Assignment warns (soft) when the assignee already has an exam-duty obligation on the same day. Bulk assignment fans out into individual trackable copies — there are no shared group tasks; committee work is modeled as a task to the convener. Visibility is vertical: assigner, assignee, the chain above the assigner, plus the assignee's unit head (HoD for Department members; registry unit head for non-departmental staff) — peers never see each other's tasks. Assigners see per-Faculty-Member open-task counts to distribute work fairly. Completion is verified by the assigner, who marks completed or reopens with comments. Tasks also carry **tags** — free-form, School-scoped grouping labels distinct from the kind taxonomy: shared metadata on the task, editable by assigner and assignee, visible to everyone who can see the task, filterable on every dashboard, inherited by bulk copies and recurrence cycles, and curated (rename/merge/deactivate) by the School Incharge.
...
- VC/Registrar as assigners — leadership delegates through Principals, Faculty Deans, and School Incharges (locked). They appear in this module only as escalation recipients.
...
|---|---|
| Principal / Faculty Dean / School Incharge | Create/assign tasks to HoDs and Faculty Members within their campus/School scope; verify completion; reopen; campus/School overview dashboard; workload counts |
...
| VC / Pro-VC office | Not assigners. Receive escalation notifications for overdue tasks assigned by top-level assigners (Principals, Dean Academic Affairs) (top of the escalation chain); no task read access beyond the escalation summary (title, assignee, due date, overdue days) |
...
|---|---|---|
| Create/assign task | Principal/Faculty Dean/School Incharge/HoD (and scoped Exam Cell grant) — only to users **below them** in the hierarchy AND within their org-unit scope | API + service layer (hierarchy + scope check) |
...
| Configure task-kind taxonomy | School Incharge (own School); seeded defaults editable, kinds with live tasks cannot be deleted (deactivate only) | API + service layer |
| Add/remove tags on a task | Assigner and assignee, until the task completes; every change audited | API |
| Create a new tag (free-form, own School) | Anyone who may tag a task; case-insensitive dedupe against existing tags | API + service layer |
| Curate School tag catalog (rename / merge / deactivate) | School Incharge or School Admin (own School); deactivate-only, never delete | API + service layer |
...
5. Escalation: N days after due (N School-configurable, default 2), an overdue task auto-notifies the assigner and the assigner's superior; repeated daily until state changes. **Top of chain:** when the assigner's chain superior would be the VC or higher (a Principal, or Dean Academic Affairs), the escalation's "superior" recipient is the **VC/Pro-VC office** (configured recipient list at university level) — escalation never dead-ends. VC office recipients get the escalation summary only, not task access. **Non-hierarchy assigners:** for assigners outside the Principal/Dean/HoD chain (e.g., an Exam Cell scoped grant), the escalation "superior" is the **unit head named on the assigner grant** (defaulting from the AUTH role-registry unit-head map — Exam Cell → Registrar); every scoped assigner grant must resolve to an escalation recipient at grant time.
...
9. Visibility (vertical + unit-head transparency): assigner, assignee, and the chain above the assigner within scope (e.g., a School Incharge sees tasks an HoD under them assigned) — **plus the assignee's unit head**: the HoD for Department members, and for non-departmental staff (Exam Cell clerks, office staff) the unit head resolved from the AUTH role-registry unit-head map (e.g., Exam Cell → Registrar; Department office staff → HoD). The unit head has read visibility of every task assigned to their unit's members from above (skip-level assignments included). Peers of the assignee see nothing; the assignee's own reports see nothing.
9a. **Task-kind taxonomy is School-configurable:** each School owns its kind list (seeded with administrative duty, exam duty, event coordination, report submission). Kinds are managed by the School Incharge; a kind with live tasks can be deactivated (hidden for new tasks) but never deleted, so history stays reportable. Cross-School reporting groups by kind label; the university accepts that Schools' taxonomies may diverge.
...
13. **Tags (locked 24-07-2026):** a task carries 0–10 tags — shared metadata, one set per task, visible to everyone who can see the task (viewers read-only; tags never widen task visibility). Tags are School-scoped entities: names ≤ 30 chars, `[a-z0-9-]`, case-insensitive unique per School; anyone who may tag may create one on the fly (autocomplete reuses existing names). Assigner and assignee add/remove tags until completion (audited); completed tasks keep their tags (reportable, not editable). Bulk-assign copies and recurrence cycle instances inherit the definition's tags; each copy's tags are independently editable afterward, and tags follow the task through decline/reassignment. Curation is **entity-semantic**: School Incharge/School Admin rename (reflected everywhere, history in audit), merge (losing tag's tasks re-pointed to the winner; a task pushed past 10 tags by a merge is allowed and flagged in the merge summary), and deactivate (hidden from pickers, retained and filterable on existing tasks — never deleted). Concurrent tag edits: add/remove are independent per-tag operations — no lost updates. Tags are distinct from the kind taxonomy (kind: mandatory, single, School Incharge-governed; tags: optional, multiple, user-created). **Tag names must contain no personal data or sensitive matter** (rule surfaced as a UI hint; School Incharge curation is the cleanup path) since names are visible School-wide via autocomplete. Dashboards gain tag filter/group within the viewer's existing scope; tag-based reports/exports (status breakdown per tag) are School-scoped. Reminder/escalation rules by tag: out of scope (post-MVP).
...
- **DPDP — tags:** tags are non-personal organizational metadata. Tag *names* are visible School-wide (autocomplete/picker) even to users who cannot see the underlying tasks, so names must contain no personal data or sensitive matter (§4 rule 13 — UI hint + School Incharge curation as cleanup); tag-filtered views and reports never expose tasks beyond the viewer's existing scope.
...
**US-TSK-4** — As a Faculty Dean, I bulk-assign exam invigilation duty to 40 Faculty Members so that each is individually accountable.
...
- TSK-FR-05: Overdue escalation: N days after due (configurable, default 2) notify assigner + assigner's superior (chain superior; unit head per §4 rule 5 for non-hierarchy assigners); repeat daily until status changes.
...
- TSK-FR-10: Visibility per §4 rule 9: assigner, assignee, chain above assigner within scope, plus read visibility for the assignee's unit head (HoD for Department members; registry unit head for non-departmental staff) on all tasks assigned to their unit's members; peer and downward visibility denied by default.
...
- TSK-FR-14: Full recurrence (MVP, per §4 rule 9c): fixed intervals with schedule anchor, holiday-aware shifting via the calendar resolver (TTM-FR-28) — resolved **for the assignee's School**, since a Nursing School teaching Sunday and an Engineering School closed on it disagree about which dates are working, optional ordered assignee-rotation pool with skip-and-flag for ineligible members, end conditions (until-date / N cycles / open-ended), stop toggle; each cycle instantiates an independent task.
...
- TSK-FR-17: Same-day exam-duty conflict warning at assignment (single and bulk): soft warning naming the conflicting obligation; assigner acknowledgment required and recorded; never blocks. In addition to open exam-duty tasks, the warning consults the School academic calendar's exam-date ranges (TTM-FR-18) when the assignee's School has an approved calendar — also as a soft signal only.
- TSK-FR-18: School task-kind taxonomy management by the School Incharge: create/rename/deactivate kinds; kinds with live tasks deactivate-only; seeded with the four default kinds.
- TSK-FR-19: Escalation top-of-chain: university-level configured VC/Pro-VC office recipient list receives escalations for tasks assigned by top-level assigners (Principals, Dean Academic Affairs); recipients get summaries, not task access.
- TSK-FR-20: **Task tags** per §4 rule 13: 0–10 School-scoped tags per task; free-form creation with case-insensitive autocomplete dedupe; add/remove by assigner and assignee until completion, audited; visible read-only to everyone with task visibility; inherited by bulk copies and recurrence cycles (independently editable per copy); dashboard tag filter/group within viewer scope; School-scoped tag reports/exports; name validation (≤30 chars, `[a-z0-9-]`) with field-level errors, 11th tag rejected.
- TSK-FR-21: **School tag curation** by School Incharge/School Admin: rename (entity-wide, audited with old name), merge (re-point tasks to survivor; over-limit results allowed and flagged in the merge summary), deactivate (hidden from pickers, retained on tasks, filterable); no delete; all curation actions audited.
...
| Recurring cycle lands on a non-working date (university holiday or a weekday the School does not work) | **DECISION:** due date shifts to the next working day (School-overridable to previous working day); the shift is shown on the task. |
...
| Concurrent tag edits by assigner and assignee | **DECISION:** add/remove are independent per-tag operations — both apply; no lost updates; audit records each actor's change in order. |
| Tag merge pushes a task past the 10-tag cap | **DECISION:** allowed — curation wins over the cap; the affected tasks are listed in the merge summary; users cannot manually add an 11th tag afterwards until below the cap. |
| Tag name containing personal/sensitive content (e.g., 'disciplinary-drrao') | **DECISION:** policy rule (no personal data in tag names) surfaced as a UI hint; not machine-enforced in MVP; School Incharge rename/merge is the remedy, audited. |
| Tag sprawl despite autocomplete | **DECISION:** School Incharge/School Admin merge + deactivate are the cleanup tools (entity semantics — reports heal retroactively); no automatic similarity policing in MVP. |
...
- The role hierarchy (Principal/Faculty Dean/School Incharge > HoD > Faculty Member) and org-unit scoping are resolvable from AUTH role grants; TSK performs no hierarchy bookkeeping of its own.
...
- Working days come from the TTM calendar resolver (TTM-FR-28): the **university holiday calendar** (System Admin) layered under each **School's working-day pattern** (School Incharge). This module reads it for holiday-aware recurrence and always resolves against the assignee's School — there is no single university-wide working week; exam-duty obligations for the conflict warning are resolvable from open exam-duty tasks in this module (MVP scope — timetable-level invigilation data is not consulted).
...
flowchart TD
  ...
  C3 --> D
  C2 -- No --> D[Task assigned · assignee notified · assignee's unit head gains read visibility]
...
| TC-TSK-010 | Bulk assign 40 targets creates 40 independent copies | Happy | P0 | Faculty Dean, 40 in-scope Faculty Members | Bulk assign | 40 tasks, independent statuses; fan-out summary shown | TSK-FR-09, US-TSK-4 |
...
| TC-TSK-013 | Chain above assigner sees the task | Access | P1 | HoD-assigned task; School Incharge above HoD | School Incharge opens task | Visible read-only per chain visibility | TSK-FR-10 |
...
| TC-TSK-021 | Assignee's HoD sees skip-level task | Access | P0 | Faculty Dean assigns directly to a Faculty Member in Dept-A | Dept-A HoD opens department task view | Task visible read-only to the HoD; peers still see nothing | TSK-FR-10, §4 rule 9 |
...
| TC-TSK-023 | Recurring cycle shifts off a holiday | Boundary | P0 | Monthly recurrence; next cycle lands on a university holiday | Cycle instantiates | Due date = next working day; shift shown on task | TSK-FR-14, §8 |
...
| TC-TSK-027 | Kind with live tasks deactivate-only | Negative | P1 | Kind "event coordination" has 12 live tasks | School Incharge attempts delete, then deactivate | Delete impossible; deactivate hides kind from picker; live tasks unchanged and reportable | TSK-FR-18, §8 |
...
| TC-TSK-029 | Exam-Cell-assigned overdue task escalates to Registrar | Access | P0 | Exam Cell assigner grant (unit head = Registrar); task N days overdue | Escalation job runs | Assigner + Registrar notified (summary); repeats daily; no VC-office involvement | TSK-FR-05, §4 rule 5 |
| TC-TSK-030 | Unit head sees non-departmental assignee's task | Access | P1 | Registrar's office assigns a task to an Exam Cell clerk | Registrar (clerk's registry unit head) opens unit task view; a peer clerk queries the task | Registrar sees it read-only; peer gets 404/403 | TSK-FR-10, §4 rule 9 |
| TC-TSK-031 | Assigner tags at creation; assignee sees and adds a tag | Happy | P0 | HoD assigns task tagged `naac-audit` to Dr. Rao | 1. Rao opens the task 2. Rao adds tag `week-3` | Rao sees `naac-audit`; both tags now visible to HoD and Rao; both changes audited | TSK-FR-20, §4 rule 13 |
| TC-TSK-032 | Case-insensitive dedupe on tag creation | Boundary | P0 | Tag `exam-2026` exists in the School | Assignee types `EXAM-2026` on another task | Existing tag reused — no second entity; autocomplete offered it during typing | TSK-FR-20 |
| TC-TSK-033 | Tag limits enforced | Negative | P1 | Task has 10 tags | 1. Add an 11th tag 2. Create tag `exam 2026!` (space, punctuation) 3. Create a 31-char name | Each rejected with a field-level error naming the rule (cap / charset / length) | TSK-FR-20, §4 rule 13 |
| TC-TSK-034 | Curation entity semantics: rename, merge, deactivate | Happy | P0 | Tags `exam-2026` (12 tasks), `Exam2026` (3 tasks), `old-drive` (5 tasks incl. 2 completed) | School Incharge renames `exam-2026`→`sem2-exams-2026`; merges `Exam2026` into it; deactivates `old-drive` | All 15 tasks show `sem2-exams-2026`; reports heal retroactively; `old-drive` gone from pickers but still on and filterable for its 5 tasks; every action audited with prior names | TSK-FR-21, §8 |
| TC-TSK-035 | Bulk copies and recurrence cycles inherit tags; scope respected in tag views | Access | P0 | Faculty Dean bulk-assigns 40 copies tagged `exam-2026`; monthly recurrence tagged `naac-audit`; viewer is an HoD of one Department | 1. Inspect copies and next cycle instance 2. HoD filters department dashboard by `exam-2026` | All copies + cycle instance carry the tags, each independently editable; HoD's tag filter returns only their scope's tasks — no widened visibility | TSK-FR-20, §4 rule 13, §5 |

Coverage: every §6 acceptance criterion, the §4 authorization matrix (downward-only, scope, delegation, visibility incl. unit-head skip-level and VC exclusion), all §8 decisions including recurrence/holiday/rotation, taxonomy, and tag rules (TC-031–035 cover tagging rights, dedupe, limits, curation entity semantics, inheritance, and scope-safe tag views), the DPDP visibility-minimization and tag-name rules, and the headline NFRs map to at least one test. The bulk-cap (200) worst case is covered implicitly by TC-TSK-010/011 tooling limits — add an explicit cap test during implementation. Tag-merge over-limit flagging (§8) is exercised via TC-TSK-034's merge step — extend with an over-limit fixture during implementation.
