...
Student onboarding in Tasq is **import-only**: students arrive fully formed from the university's existing ERP, either as bulk files (CSV) or via an ERP API feed. Tasq does **not** admit students — it **provisions** them: creates the user account, delivers initial credentials (SMS primary, email fallback), maps each student to campus/School/Department/Program/Section, records the roll number, kicks off device registration on first login (per 01-authentication-authorization-security.md), and enforces the DPDP minor/parental-consent gate before activation. The module also covers the ongoing lifecycle: mid-term single-student additions, section allotment and re-allotment, campus/program transfers, withdrawal/dropout deactivation, and data-correction grievances.
...
- Minor flag + parental-consent artifact check before a minor's account activates (DPDP).
...
- Editing ERP-mastered fields (name, date of birth, ERP ID) inside Tasq — corrections round-trip through the ERP.
...
| Record/verify parental-consent artifact | Admin/office staff (own campus), System Admin | API |
| Approve data-correction grievance | Admin/office staff for Tasq-owned fields; ERP-mastered fields routed to ERP with tracked status | API |
...
3. A student is created in state `IMPORTED`; moves to `ACTIVE` only after (a) credential delivery initiated and (b) the minor/consent gate passes. Minors without a recorded parental-consent artifact stay `PENDING-CONSENT` (AUTH-FR-11).
...
Every batch (who, when, file hash, row counts: created/updated/rejected), every single add, re-allotment, transfer, withdrawal, and consent-artifact record writes to the central audit service with before/after snapshots and reason where applicable.

## 5. Legal & Regulatory Requirements

- **DPDP data minimization:** the import schema carries only fields Tasq needs (identity, contact for OTP/credentials, org mapping, DOB for minor determination, consent references). Fee, caste/category, and other ERP fields are not imported.
...
- **Minors:** DOB in the import determines the minor flag. A minor's account cannot reach `ACTIVE` without a parental-consent artifact reference (imported from ERP or recorded by office staff). Turning 18 auto-clears the flag; the historical consent record is retained.
- **Right to correction:** grievances on Tasq-owned fields (contact number, section-display data) are correctable in Tasq; ERP-mastered fields are forwarded to the ERP and the grievance status tracks the round-trip — the student always receives a response, never silence.
...
- Given valid single-student data with an ERP ID, when I submit, then the same validation, consent gate, and credential flow run as for bulk rows.
...
**US-ONB-6** — As a minor Student's record, I must not activate without parental consent. (Legal)
- Given an imported student aged 17 with no consent artifact, when activation is attempted, then the account stays `PENDING-CONSENT`, no credentials are sent, and the campus consent-pending queue lists the student.
...
- ONB-FR-06: Account provisioning into `IMPORTED` state; activation pipeline: consent gate → credential generation → delivery (SMS primary, email fallback) → `ACTIVE`.
- ONB-FR-07: Minor flag computed from DOB; parental-consent artifact reference stored; activation blocked without it; consent-pending queue per campus.
...
- ONB-FR-13: Data-correction grievance handling: Tasq-owned fields correctable with audit; ERP-mastered fields routed to ERP with tracked status and user-visible outcome.
...
| Row's ERP ID exists but name/DOB wildly differ | Update is applied (ERP is master) but flagged `identity-warning` in the batch report for human review; DOB change that flips the minor flag re-runs the consent gate. |
...
- The ERP export includes: ERP ID, name, DOB, gender, mobile, email, campus/School/Department/Program codes, admission year, roll number, minor's parental-consent artifact reference. Section may be assigned in Tasq if absent from the file.
- The admission-time ERP notice covers the disclosure of student data to Tasq for provisioning; Tasq's own DPDP consent is captured at first login (AUTH doc).
- ERP is the system of record for identity fields and roll numbers; Tasq is the system of record for account state, Section membership, and device registration.
...
- **Roll-number source:** does Tasq assign roll numbers or import ERP-issued ones? **Proposed default (used throughout this doc): import ERP-issued roll numbers**; Tasq only enforces uniqueness. If any School needs Tasq-side assignment, that becomes a configurable per-School numbering rule — decide before build.
- ERP API feed: push (ERP calls Tasq) or pull (Tasq polls)? Proposed: pull on schedule + manual trigger, to keep Tasq in control of validation timing.
...
flowchart TD
  ...
  E2 --> F{Minor without consent artifact?}
  E1 --> G
  F -- Yes --> F1[Hold PENDING-CONSENT · queue for office staff]
  F -- No --> F2[Generate credentials]
  F1 -. artifact recorded .-> F2
  F2 --> H{SMS delivered?}
...
| TC-ONB-007 | Minor without consent stays pending | Legal | P0 | Row: age 17, no consent reference | Run import; attempt activation | `PENDING-CONSENT`; no credentials sent; appears in consent queue | ONB-FR-07, US-ONB-6 |
| TC-ONB-008 | Consent artifact unblocks activation | Legal | P0 | TC-ONB-007 state | Staff records artifact reference | Activation resumes; credentials delivered; both steps audited | ONB-FR-07 |
...
Coverage: every §6 acceptance criterion, the §4 authorization matrix (TC-011/014), DPDP minor-consent obligations (TC-007/008), and all §8 decisions map to at least one test except the ERP corrupted-feed guardrail and grievance round-trip, which are covered in the integration test phase.
