...
| Working pattern changed mid-term | **DECISION:** already-generated Sessions and captured attendance stand as historical fact. Note precisely where that holds: the **resolver is a pure function of current configuration**, so asking it about a past date after a pattern change does return the new answer. History is protected by ATT materialising dated Sessions when they occur, plus the rule 13 guards that stop a taught day being withdrawn underneath them — not by the resolver carrying an as-of date. Anything needing the historical answer reads the Session, not the calendar. |
| Holiday range overlaps another holiday range | **DECISION:** allowed and idempotent — a date covered by two entries is simply non-working; the resolver names the **narrower** entry as the deciding one, since it is the more specific answer (Republic Day, not "Semester break"). Overlaps happen legitimately and are not worth refusing. |
| A holiday's campus tags are widened after the fact (tag dropped, or a campus added) | **DECISION:** this newly closes the whole range for campuses that were open, so it passes the same rule 13 attendance guard as extending the dates would. Narrowing the tag list re-opens dates and is always allowed. |
...
- Venues: org-owned, University-level with a campus code, carrying capacity and kind (classroom/lab/seminar/auditorium/workshop/**field**). `field` covers off-campus sites — a PHC used for community-health field work, a partner farm — which are still venues: two cohorts cannot be at the same site in the same slot, so field work is clash-checked like any other class (05-08-2026). Clash detection is university-wide, so rooms cannot be School-owned.
...
| TC-TTM-061 | Fifth Saturday is not among "1st and 3rd" | Boundary | P1 | Pattern: Saturdays 1st and 3rd; a month with five | Resolve all five | 1st and 3rd teach; 2nd, 4th and 5th do not | TTM-FR-27, §8 |
| TC-TTM-062 | Overlapping holidays: narrower decides | Boundary | P1 | A public holiday inside a vacation block | Resolve a date in each | Both non-working; the overlapped date names the public holiday, not the block | §8 |
| TC-TTM-063 | School works part of a vacation block | Boundary | P1 | 14-day vacation; School declares 2 days working | Resolve the block | Only the 2 declared dates teach, decided by school-override | TTM-FR-27, §8 |
| TC-TTM-064 | Holiday on an already non-teaching day | Boundary | P2 | Mon–Fri School; holiday on a Sunday | Declare it, resolve | Accepted; the date was and stays non-working; no exception needed | §8 |
| TC-TTM-065 | Widening a holiday's campus tags is guarded | Negative | P0 | Holiday tagged to one campus; attendance captured elsewhere | Drop the tag, then add a campus | Both refused naming the campuses; narrowing the list still allowed; nothing written | §4 rule 13 |

Coverage addition: rebalancing triggers/ranking/empty-list (TC-021/022/023), the swap lifecycle incl. concurrency and one-hop (TC-024/025/026) map to TTM-FR-16/17; School calendar lifecycle and exam-date warnings (TC-027/028) map to TTM-FR-18; Section-instance dependency (TC-029, with TC-ONB-007/008) maps to TTM-FR-19. **Calendar coverage (TC-045…060)** maps to TTM-FR-26/27/28 and §4 rules 11–13: weekend-teaching Schools, alternate Saturdays, campus-tagged holidays, the four-layer precedence, compensatory days, both narrowing guards, the unconfigured-School fallback, access denial, and the range-resolution NFR. TC-061…065 close the §8 boundaries the first implementation shipped without: the five-Saturday month, overlapping ranges, partial vacation working, the no-op holiday, and the campus-tag widening that bypassed rule 13. Not yet covered: half-day working dates and per-date custom grids, both explicitly out of scope (§11); and TC-060 asserts range correctness rather than measured latency, which belongs in a load test rather than the unit suite.
